...
# label each GPS-location based on the estimated velocity and turning angle. 
...
# 'ag_dta' for Audouin's gulls (Ichthyaetus audouinii) at Deserta Island, during the breeding and 
# dispersal⁄ migration period;
ag <- read.csv('ag_dta.csv', header=TRUE, sep=",")
names(ag) # to get the headers of an object
head(ag) # to display the first rows in the input data frame
str(ag) # to display the internal structure of an object 
...
# HINT: Date column must be always in the format yyyy-mm-dd hh:mm:ss
...
# The agbc@R slot is a matrix with the values that delimit each binary region. 
...
# We can make use of the labelling values stored in agbc@A and the values of the delimiters stored in agbc@A;
# To visualize the binary splitting of velocities separately for low and high values of turns;
hist(agbc@X[which(agbc@A%in%c(1,3)),1],breaks=seq(0,max(agbc@X[,1]), max(agbc@X[,1])/50),include.lowest=TRUE,xlim=range(agbc@X[,1]), 
     xlab='velocity (m/s)',main='velocity distribution for LOW turns',cex.main=0.8)

hist(agbc@X[which(agbc@A%in%c(2,4)),1],breaks=seq(0,max(agbc@X[,1]), max(agbc@X[,1])/50),include.lowest=TRUE,xlim=range(agbc@X[,1]), 
...
hist(agbc@X[which(agbc@A%in%c(1,2)),2],breaks=seq(0,pi,pi/50),
     xlab='turn (rad)',main='turn distribution for LOW velocities',cex.main=0.8)

hist(agbc@X[which(agbc@A%in%c(3,4)),2],breaks=seq(0,pi,pi/50),
...
# If you are from the team gull, back to square one!
...
ag_with_behaviours <- cbind(ag, Behaviour = Behaviours)
head(ag_with_behaviours)
...
ag_with_behaviours <- ag_with_behaviours %>%
...
# HINT 2: get the coordinates (in decimal degrees) for Deserta or Barreta island in southern mainland Portugal;
# https://www.findlatitudeandlongitude.com

# ... and please complete the exercise first before you start looking for boat trips to Algarve ;)



DaytimeHours <- getSunlightTimes(
  date = seq.Date(as.Date("yyyy-mm-dd"), as.Date("yyyy-mm-dd"), by = 1), # tracking period
  keep = c("sunrise", "sunriseEnd", "sunset", "sunsetStart"),
  lat =YY, # colony coordinates
  lon = -XX,
  tz = "GMT")
...
ag_with_behaviours$Dategmt <- as.character(ag_with_behaviours$Dategmt)
...
ag_with_behaviours$Time <- ifelse(nchar(ag_with_behaviours$Time) == 5, 
                                  paste0(ag_with_behaviours$Time, ":00"), 
                                  ag_with_behaviours$Time)


# Convert the 'Time' column to a time object 
ag_with_behaviours$Time <- hms(ag_with_behaviours$Time)



##### QUESTION 4 #####

# We want to create a new column in our dataset that tell us whether gps positions are happening during 
# the day or night;
...
ag_with_behaviours$Time_of_day <- ifelse(ag_with_behaviours$Time < sunrise | ag_with_behaviours$Time > sunset, 
                                         "Night", "Day")


# See the changes
View(ag_with_behaviours)
...
behaviour_summary <- ag_with_behaviours %>%
  group_by(Behaviour_class, Time_of_day) %>%
  summarise(count = n(), .groups = 'drop') %>%
  complete(Behaviour_class = unique(ag_with_behaviours$Behaviour_class), 
...
# How can you get this information? How long did this gull spend resting during the day?
